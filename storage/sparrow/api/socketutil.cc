...
SocketAddress SocketAddress::anyAddr_ = SocketUtil::getAddress("127.0.0.1", 0);
...
// STATIC
my_socket SocketUtil::create(int type, const SocketAddress& src_addr) 
...
SocketAddress SocketUtil::getAddress(const char* address, uint32_t port) _THROW_(SparrowException) {
	...
	if ( !ipAddress.isV4() ) {
		if ( !v6_ ) {
			throw SparrowException::create(false, SPW_API_FAILED, "IPV6 not supported. Cannot use address %s", (address != NULL ? address : ""));
		}
		...
		if (address == 0 || strlen(address) == 0) {
...
#endif
			if (!ok) {
				throw SparrowException::create(true, SPW_API_FAILED, "Cannot convert address \"%s\" to IPv6 internal format", address);
			}
		} else {
			...
			if (code != 0) {
				throw SparrowException::create(false, SPW_API_FAILED, "Cannot get address info for \"%s\", port %u (%s)",
...
SocketAddress SocketUtil::getAddress(my_socket socket) _THROW_(SparrowException) {
	...
	if (getsockname(socket, reinterpret_cast<sockaddr*>(&socketAddress), &length) != 0) {
		throw SparrowException::create(true, SPW_API_FAILED, "Cannot get bind address of socket");
	}
...
// STATIC
void SocketUtil::notifyStopSocket() {
	if (stopSocket_ != INVALID_SOCKET) {
		...
		if (sent < 0) {
			try {
				throw SparrowException::create(true, SPW_API_FAILED, "Cannot notify stop socket");
