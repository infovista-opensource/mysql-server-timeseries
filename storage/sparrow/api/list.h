...
#ifndef _spw_api_list_h_
#define _spw_api_list_h_
...
public:
	...
	SYSslink<T>* acquire(const T& object, SYSslink<T>* next) {
		if (root_ == 0) {
			...
			if (l == 0) {
				printf("SYShPoolAllocator::acquire: cannot allocate %llu bytes of memory", static_cast<ulonglong>(sizeof(*l)));
			}
			return l;
		} else {
			SYSslink<T>* link = root_;
...
template<class T> inline void SYSidlist<T>::unlink(T* t) {
	SPW_dbgASSERT(n_ > 0);
	if (t->prev_ != 0) {
		SPW_dbgASSERT(t->prev_->next_ == t);
		t->prev_->next_ = t->next_;
	} else {
		SPW_dbgASSERT(first_ == t);
		first_ = t->next_;
	}
	if (t->next_ != 0) {
		SPW_dbgASSERT(t->next_->prev_ == t);
		t->next_->prev_ = t->prev_;
	} else {
		SPW_dbgASSERT(last_ == t);
		last_ = t->prev_;
	}
	t->prev_ = 0;
	t->next_ = 0;
...
template<class T> inline void SYSidlist<T>::link(T* t, T* right) {
	SPW_dbgASSERT(first_ != 0 && last_ != 0 && t != 0 && right != 0);
	right->prev_ = t;
	right->next_ = t->next_;
	t->next_ = right;
...
template<class T> inline void SYSidlist<T>::append(T* t) {
	if (last_ == 0) {
		SPW_dbgASSERT(first_ == 0);
		last_ = t;
		first_ = t;
		t->prev_ = 0;
		t->next_ = 0;
	} else {
		SPW_dbgASSERT(first_ != 0 && last_->next_ == 0);
		last_->next_ = t;
		t->prev_ = last_;
		t->next_ = 0;
		last_ = t;
