...
#ifndef _spw_api_hash_h_
#define _spw_api_hash_h_
...
template<class T> inline void SYShashBase<T>::extend() {
	if (!vector_.isEmpty()) {
		...
		const uint32_t buckets = vector_.length() * 2;
		vector.reshape(buckets);
		initialize(vector);
		for (uint32_t i = 0; i < vector_.length(); ++i) {
...
template<class T, class A> inline void SYShash<T, A>::clear() {
	for (uint32_t bucket = 0; bucket < this->vector_.length(); bucket++) {
...
template<class T, class A> inline T* SYShash<T, A>::insertAndReturn(const T& t) {
	...
	const uint32_t bucket = (spreadHashCode(h) % buckets);
	SYShlink<T>* sl = acquire(t, h, this->vector_[bucket]);
...
template<class T, class A> inline SYShashIterator<T, A>::SYShashIterator(const SYShash<T, A>& hash) : hash_((SYShash<T, A>&)hash) {
	reset();
}
...
template<class T, class A> inline void SYSpHash<T, A>::insert(T* t) {
	...
	this->vector_[bucket] = acquire(t, h, this->vector_[bucket]);
	this->items_++;
	if (this->items_ == this->vector_.length()) {
		this->extend();
	}
